...
define_system_module ../Utility.cpp

#
# SC_MODULEs to be synthesized
#
### 3. Add your design source files here (to be high-level synthesized).
define_hls_module TangentSearcher ../TangentSearcher.cpp

### 4. Define your HLS configuration (arbitrary names, BASIC and DPA in this example). 
define_hls_config TangentSearcher BASIC
define_hls_config TangentSearcher DPA       --dpopt_auto=op,expr

set INPUT_DIR       ".."
set INPUT_FILE_NAME "${INPUT_DIR}/input.txt"
set OUT_FILE_NAME   "result.csv"

### 5. Define simulation configuration for each HLS configuration
### 5.1 The behavioral simulation (C++ only).
define_sim_config B -argv "$INPUT_FILE_NAME $OUT_FILE_NAME"
### 5.2 The Verilog simulation for HLS config "BASIC". 
define_sim_config V_BASIC "TangentSearcher RTL_V BASIC" -argv "$INPUT_FILE_NAME $OUT_FILE_NAME"
### 5.3 The Verilog simulation for HLS config "DPA". 
define_sim_config V_DPA "TangentSearcher RTL_V DPA" -argv "$INPUT_FILE_NAME $OUT_FILE_NAME"